...
class CDQN:
    ...
    def copy_q_params(self):
        ...
        self.init_updates = [q_net_var.assign(net_var) for q_net_var, net_var in zip(self.target_network.variables, self.network.variables)]

    def update_target_params(self):
        """ update Q' networks parameters based on Q networks parameters """
        self.soft_updates = [self.soft_update(q_net_var, net_var) for q_net_var, net_var in zip(self.target_network.variables, self.network.variables)]
    ...
    def train_minibatch(self):
        ...
        for iteration in range(self.args.update_repeat):
            states, actions, rewards, dones, next_states = self.buffer.get_batch(self.args.batch_size)
            ...
            # calculate target y
            target_y = rewards + ((1 - dones) * (self.args.discount_rate * np.squeeze(target_V)))
